...
REGION_MATCH_TABLE = {
    ...
    "Rural Alberta": [
        "rural", "north", "northern alberta", "east central", "south",
        "southern alberta", "central alberta", "mountain view", "parkland"
    ],
...
def filtered_except(
    df_in, q_text, selected_regions, selected_ftypes,
...
df_except_region   = filtered_except(
    df, q, selected_regions, selected_ftypes,
    selected_famts, selected_stage, selected_activity,
...
region_counts    = {
    r: int(df_except_region[COLS["REGION"]].astype(str)
...
# centralised state for funding-type info toggles
...
def render_funding_type_filter(label, options, counts, state_prefix="ftype"):
    ...
    with st.sidebar.expander(label, expanded=False):
        if st.button("Clear", key=f"clear_{state_prefix}"):
            for opt in options:
                st.session_state[f"{state_prefix}_{opt}"] = False
            # also clear any open info panels
    ...
    # save back (dict is already mutated, but explicit for clarity)
...
if total == 0:
    ...
    for i, (_, row) in enumerate(subset.iterrows(), 1):
        ...
        badge_cls = "operational" if "operational" in s_low else (
            "open" if any(k in s_low for k in ["open", "active", "ongoing", "accepting", "rolling"])
            else "closed"
        )
        badge_label = status_raw or (
            "Operational" if badge_cls == "operational"
        ...
        with st.container():
            # marker so CSS knows to style this block as a card
