...
# Streamlit Cloud exposes secrets via st.secrets; fall back to env var
...
# Synonym pack (canonical -> set of aliases)
SYNONYMS = {
...
TAG_GROUPS = {
    "Business Stage": ["startup", "early", "growth", "scale", "export", "succession"],
    "Audience / Priority": ["women", "indigenous", "youth", "newcomer", "black", "rural", "immigrant", "veteran", "disabled"],
    "Support Type": ["grant", "loan", "financing", "funding", "tax", "rebate", "voucher", "mentorship", "training", "advisory", "accelerator"],
    "Sector / Theme": ["tech", "agri", "tourism", "cleantech", "energy", "arts", "manufacturing"],
}
...
def parse_tags_field(s: str):
    return [canon_tag(t) for t in [x.strip() for x in str(s).split(";") if x.strip()]]
...
def bucketize_tags(tag_list):
    buckets = {k: [] for k in TAG_GROUPS.keys()}
    other = []
    for t in tag_list:
        placed = False
        for group, keys in TAG_GROUPS.items():
            if any(k in t for k in keys):
                buckets[group].append(t)
                placed = True
        if not placed:
            other.append(t)
...
def is_open(status):
    return str(status).strip().lower() in OPEN_STATUSES

import re as _re
_money_pattern = _re.compile(r"(\$?\s*\d[\d,]*\.?\d*)", _re.IGNORECASE)
...
def funding_bucket_label(amount: float):
    if amount is None:
...
def load_data():
    ...
    df = ensure_cols(df)
    df["__funding_num"] = df[COLS["FUNDING"]].apply(lambda s: parse_amount(str(s)))
    df["__funding_bucket"] = df["__funding_num"].apply(funding_bucket_label)
...
# ---------- Helper: contextual counts ----------
def contextual_counts(base_df):
    reg_counts = base_df[COLS["REGION"]].fillna("").value_counts().to_dict()
    tag_counts = {}
    for s in base_df["__tags_norm"].fillna("").astype(str):
        for t in [x for x in s.split(";") if x]:
...
with tab_browse:
    ...
    regions_all = ["All Regions"] + regions_list
    ...
    labels = [b[0] for b in FUNDING_BUCKETS]
    ...
    if sel_funding:
        df_base = df_base[df_base["__funding_bucket"].isin(sel_funding)]

    with st.sidebar.expander("Tags by Category", expanded=True):
        tags_all = all_tags(df)
        buckets, other = bucketize_tags(tags_all)
        if "tag_checks" not in st.session_state:
            pre = set(qp_get("tags","").split("|")) if qp_get("tags","") else set()
            st.session_state.tag_checks = {opt: (opt in pre) for opt in tags_all}
        _, tag_counts_ctx = contextual_counts(df_base)
        for group_name, options in buckets.items():
        ...
        if colT2.button("Clear tags"):
            for opt in tags_all: st.session_state.tag_checks[opt] = False
            st.experimental_rerun()
    sel_tags = [opt for opt, checked in st.session_state.tag_checks.items() if checked]
    if sel_tags:
        df_base = df_base[df_base["__tags_norm"].apply(lambda s: all(t in s.split(';') for t in sel_tags))]

    with st.sidebar.expander("Geographic Region", expanded=True):
        regions = regions_list
        if "reg_checks" not in st.session_state:
            pre = set(qp_get("regions","").split("|")) if qp_get("regions","") else set()
            st.session_state.reg_checks = {opt: (opt in pre) for opt in regions}
    ...
    if st.sidebar.button("Reset all filters"):
        ...
        st.session_state.tag_checks = {opt: False for opt in all_tags(df)}
    ...
    def apply_filters(df_in):
        ...
        if not include_closed:
            out = out[out[COLS["STATUS"]].apply(is_open)]
        if st.session_state.pill_region != "All Regions":
            out = out[out[COLS["REGION"]].astype(str) == st.session_state.pill_region]
        if sel_regions:
            out = out[out[COLS["REGION"]].isin(sel_regions)]
        if sel_tags:
            out = out[out["__tags_norm"].apply(lambda s: all(t in s.split(';') for t in sel_tags))]
        if sel_funding:
    ...
    key_param = qp_get("key")
    if key_param:
        fdf = fdf[fdf[COLS["KEY"]].astype(str) == key_param]
    ...
    def permalink_for(row):
        params = st.experimental_get_query_params()
        params["key"] = [str(row[COLS["KEY"]])]
        order = ["q","closed","sort","layout","pill","regions","tags","fund","ps","pg","key"]
        qpairs = []
        for k in order:
            v = params.get(k, [""])
            qpairs.append(f"{k}={v[0]}")
        return "?" + "&".join(qpairs)
    ...
    if page_df.empty:
        ...
        for i, (_, row) in enumerate(page_df.iterrows()):
            with cols[i % cols_per_row]:
                ...
                status_class = "open" if status_val in OPEN_STATUSES else "closed"
                status_label = row[COLS["STATUS"]]
                desc = str(row[COLS["DESC"]]) or "No description provided."
                tags_txt = " • ".join(parse_tags_field(row[COLS["TAGS"]])) or "—"
                ...
                links = []
                if website: links.append(f"<a href='{website}' target='_blank'>Website</a>")
                if email: links.append(f"<a href='mailto:{email}' target='_blank'>Email</a>")
                if phone: links.append(f"<a href='tel:{phone}' target='_blank'>Call</a>")
                if contact: links.append(f"<a href='{contact}' target='_blank'>Contact</a>")
                ...
                st.markdown(f"<div class='card'>"
                            ...
                            f"<div class='tags'>Tags: {tags_txt}</div>"
                            f"<div class='tags'>Funding: {funding_label}</div>"
                            f"<p>{desc}</p><div class='links'>{links_html}</div>"
                            f"</div>", unsafe_allow_html=True)
                c1b, c2b = st.columns(2)
                with c1b:
                    heart = "Remove Favorite" if keynorm in st.session_state.favorites else "Add to Favorites"
                    if st.button(heart, key=f"fav_{keynorm}"):
                        if keynorm in st.session_state.favorites: st.session_state.favorites.remove(keynorm)
                        else: st.session_state.favorites.add(keynorm)
                with c2b:
                    st.text_input("Permalink", value=permalink_for(row), key=f"plink_{keynorm}")
...
with tab_favs:
    ...
    if not st.session_state.favorites:
        ...
        csv = fav_df.drop(columns=[c for c in fdf.columns if str(c).startswith('__')], errors="ignore").to_csv(index=False).encode("utf-8") if 'fdf' in locals() else fav_df.to_csv(index=False).encode("utf-8")
        st.download_button("Download Favorites CSV", csv, file_name="favorites.csv", mime="text/csv")
        for _, row in fav_df.iterrows():
            st.markdown(f"- {row[COLS['PROGRAM_NAME']]} — {row[COLS['ORG_NAME']]}")
...
with tab_suggest:
    ...
    with st.form("suggest_form", clear_on_submit=True):
        ...
        s_tags = st.text_input("Meta Tags (semicolon-separated)")
