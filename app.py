...
index ecbafd2426574eeb67434546d626c46a02134e40..b4e263e31a021948abde31e64ff670317c75c0a6 100644
--- a/app.py
+++ b/app.py
@@ -1,92 +1,1619 @@
...
-# Audience: match if any audience term is found in meta tags
-audience_input = st.sidebar.text_input("Audience (search Meta Tags)")
-# Eligibility: multiselect exact values
-eligibility = st.sidebar.multiselect("Eligibility", df["Eligibility Description"].dropna().unique())
...
-if audience_input:
-    audience_search = audience_input.lower().strip()
-    filtered_df = filtered_df[filtered_df["Meta Tags"].fillna("").str.lower().str.contains(audience_search)]
+inline_gov_css()
+
+# ---------------------------- Header / Logo ----------------------------
+def embed_logo_html():
...
-if eligibility:
-    filtered_df = filtered_df[filtered_df["Eligibility Description"].isin(eligibility)]
...
-  <p>{row.get("Program Description", "No description")}</p>
-  <div class="actions-row actions-links">
-    <a href="{row.get("Program Website", "#")}" target="_blank">Website</a>
-    <a href="mailto:{row.get("Email Address", "")}">Email</a>
-    <a href="tel:{row.get("Phone Number", "")}">Call</a>
-    <a href="#">Favourite</a>
+st.markdown(
+    f"""
+<div class="header goa-header goa-app-header">
...
+
+    match = None
+    for match in re.finditer(r"(\d+(?:\.\d+)?)(?:\s*(k|m|b|million|billion|thousand))?", s, flags=re.I):
+        pass
+
+    if not match:
+        return "Unknown / Not stated"
+
+    num_part, suffix = match.group(1), (match.group(2) or "").strip().lower()
+
+    try:
+        val = float(num_part)
+    except ValueError:
